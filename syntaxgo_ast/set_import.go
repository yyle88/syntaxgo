...
type PackageImportOptions struct {
	pkgPaths        []string       // List of package paths. // 直接设置包路径列表
...
func (param *PackageImportOptions) SetPkgPath(pkgPath string) *PackageImportOptions {
	param.pkgPaths = append(param.pkgPaths, pkgPath)
	return param
}

// SetPkgPaths adds some package paths to the list of packages to be imported.
// SetPkgPaths 将包路径添加到待导入的包列表中。
func (param *PackageImportOptions) SetPkgPaths(pkgPaths []string) *PackageImportOptions {
	param.pkgPaths = append(param.pkgPaths, pkgPaths...)
	return param
}
